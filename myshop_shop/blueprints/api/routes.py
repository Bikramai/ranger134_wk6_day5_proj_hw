...
# It will be a series of endpoints that external applications can make requests to to create, read, update, and delete data.


from flask import Blueprint, request, jsonify 
from flask_jwt_extended import create_access_token, jwt_required 
from myshop_shop.models import Customer, Product, ProdOrder, Order, db, product_schema, products_schema #change object to dictionary


                        #passing the import name here
...
@api.route('/order/create/<cust_id>', methods = ['POST']) #CREATE is usually paired with a POST method (we are creating data in our database)
@jwt_required()
def create_order(cust_id):
    ...
    if not customer: #if we dont have a customer, this is their first order and lets add them in the database
        ...
        db.session.add(customer) #add them to database
    ...
    for product in customer_order:
        ...
        db.session.add(prodorder) #add them to database

        order.increment_ordertotal(prodorder.price) #it will keep increment our order
