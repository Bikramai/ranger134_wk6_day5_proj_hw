...
#internal import
from myshop_shop.models import Product,Customer, Order, db
from myshop_shop.forms import ProductForm
...
def shop():
    ...
    allprods = Product.query.all() #the same as SELECT * FROM products, list of objects means we are query the database and grabing all my products, here allprods stands for list of object
    ...
    our_class = "Marvels are the best movie ever!"
...
def create():
    ...
    if request.method == 'POST' and createform.validate_on_submit():
        ...
        character_name = createform.name.data
        ...
        product = Product(price, quantity, character_name )

        db.session.add(product) #adding our new instantiating object to our database
        db.session.commit()

        flash(f"You have successfully created product {character_name}", category='success')
        return redirect('/')
...
def update(id): #passing routes to function
