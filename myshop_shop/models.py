...
ma = Marshmallow() #makes marshallow object
...
class User(db.Model, UserMixin): 
    ...
    def __init__(self, username, email, password, first_name="", last_name=""):
        # why we have first_name="", last_name="" --- this are called default parameters or optional parameters
        # we can take an argument or we cannot
    ...
    def __repr__(self): #we just want to print our object after we created just to comfirm in our termial to varify 
        return f"<User: {self.username}>"
    


#This database needs to be changed or updates everytime depends on what project you are working
class Product(db.Model): #db.Model helps us translate python code to columns in SQL 
    ...
    character_name = db.Column(db.String(2000), nullable=False)
    ...
    def __init__(self, price, quantity, character_name): #self is the current insticiate of our class or objrct we currently in
        self.prod_id = self.set_id()                       #if we are inside of class we use self keyword. if we outside of class we do (product = Product())
    ...
    def set_id(self):
            return str(uuid.uuid4()) #directly came from the python generating unique id token for us 
    
    def get_homeworld(self, name):

        get_both = get_character(name)#name is going to become search & we are going to use it to make our api call 
...
class ProductSchema(ma.Schema): # inherit ma.Schema here ma stands for marshallow that we instantiated and calling class upon to Schema.

    class Meta:
        fields = ['prod_id', 'character_name', 'image', 'description', 'price', 'quantity']
